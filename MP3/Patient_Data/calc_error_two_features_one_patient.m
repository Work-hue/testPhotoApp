function [joint_error_table_array, ml_error, map_error, avg_error] = calc_error_two_features_one_patient(pat, f1,f2)
% selected_features_array is a 9*2 array

Joint_HT_table = cell(1,1);

for i = 1:1
    Joint_HT_table{1,i} = joint_ht(pat,f1,f2);
end

% TASK 3.2A

joint_ml_vecs = cell(1,1);
joint_map_vecs = cell(1,1);

for i = 1:1
    
    jht = Joint_HT_table{1,i};
    test_data_size = size(pat.test_data,2);
    ...
    for j = 1:test_data_size
        ...
        idx1_arr = find(jht(:,1)==pat.test_data(f1,j));
        ...
        if size(idx1_arr,1)==0
            ...
            % feature idx f2
            idx2_arr = find(jht_f1(:,2)==pat.test_data(f2,j));
...
joint_error_table_array = cell(1,1);

for i = 1:1
    mat = zeros(2, 3);
    
    % ML rule
    [probFalse, probMiss, probError] = CompareVoter(joint_ml_vecs{1,i}, pat.test_labels);
    mat(1,1) = probFalse;
    mat(1,2) = probMiss;
    mat(1,3) = probError;
    
    % MAP rule
    [probFalse, probMiss, probError] = CompareVoter(joint_map_vecs{1,i}, pat.test_labels);
    mat(2,1) = probFalse;
    mat(2,2) = probMiss;
    mat(2,3) = probError;
...
ml_error = ml_error / 1.0;
map_error = map_error / 1.0;
