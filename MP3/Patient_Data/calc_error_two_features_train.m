function [joint_error_table_array, ml_error, map_error, avg_error] = calc_error_two_features_train(patient, selected_features_array)
...
for i = 1:9
    ...
    train_data_size = size(patient(i).train_data,2);
    
    joint_ml_vecs{1,i} = zeros(1,train_data_size);
    joint_map_vecs{1,i} = zeros(1,train_data_size);
    
    for j = 1:train_data_size
        
        % find rows in Joint_HT_table corresponding to feature idx f1
        idx1_arr = find(jht(:,1)==patient(i).train_data(f1,j));
        ...
        if size(idx1_arr,1)==0
            ...
            % feature idx f2
            idx2_arr = find(jht_f1(:,2)==patient(i).train_data(f2,j));
...
for i = 1:9
    ...
    % ML rule
    [probFalse, probMiss, probError] = CompareVoter(joint_ml_vecs{1,i}, patient(i).train_labels);
    mat(1,1) = probFalse;
    mat(1,2) = probMiss;
    mat(1,3) = probError;
    
    % MAP rule
    [probFalse, probMiss, probError] = CompareVoter(joint_map_vecs{1,i}, patient(i).train_labels);
    mat(2,1) = probFalse;
    mat(2,2) = probMiss;
    mat(2,3) = probError;
...
ml_error = ml_error / 3.0;
map_error = map_error / 3.0;
